...
function App() {
    ...
    useEffect(() => {
        setNotesOnSlider(duration ? notes.map((note, idx) => <NoteMark duration={duration} seconds={note.noteTime} color={colors[note.colorId-1]} onNoteSelect={onNoteSelect} key={idx} />) : null)
    }, [notes, duration])
    ...
    const getUrlDomain = url => url ? (new URL(url)).hostname.replace('www.', '').split('.')[0] : null;
    ...
    const urlSubmit = (url) => {
        ...
        if(urlDomain === 'rutube'){
            ...
            const fetchUrl = `https://api.allorigins.win/get?url=https://rutube.ru/api/video/${rutubeId}`
            ...
            fetch(fetchUrl)
                ...
                .then(e => addNewVideo(e, url))
                .catch(e => {
                    console.error(`fetch error`, fetchUrl, e)
                    clearCurrentVideo(url);
                });
        }
        else{
    ...
    const fetchVideoData = (urlDomain, url) => {};

    const clearCurrentVideo = (url) => {setCurrentVideo({id: null, chapterId: null, added:null, title: 'Видео не выбрано', url: url});}

    const addNewVideo = (e, url) => {
        console.log('fetched video data', e, url)
        if('error' in e){
                ...
                <Grid item xs={12}>
                    <Grid container spacing={3}>
                        <Grid item lg={8} xs={12}>
                            ...
                            <VideoPlayer getUrlDomain={getUrlDomain} url={currentVideo.url} notesOnSlider={notesOnSlider} durationCallback={setDuration} setCurrentTime={setCurrentTime} ref={videoPlayerRef} />
