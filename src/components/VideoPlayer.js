...
class VideoPlayer extends Component {
    ...
    render () {
        const notesOnSlider = this.props.notesOnSlider
        const { playing, controls, light, volume, muted, loop, played, loaded, duration, playbackRate, pip } = this.state
        const currentDomain = this.props.getUrlDomain(this.props.url);
        return (
            <div className='app'>
                <section className='section'>

                    <div style={{ position: 'relative', paddingTop: '56.25%', background: '#333' }}>
                        {currentDomain === 'rutube' ? 
                        <RutubePlayer
                        ref={this.ref}
                        width='100%'
                        height='100%'
                    ...
                    <Slider sx={{ position: 'relative', zIndex: '10' }} size='medium' value={played} min={0} max={1} step={0.001} onChange={this.handleSeekChange} onChangeCommitted={(e, value) => this.handleSeekMouseUp(e, value * duration)}  valueLabelDisplay="auto" valueLabelFormat={this.timeLabelFormat}/> 
